...
```mermaid
flowchart TD
    A[Environment and observation] --> B[Compression and expression]
    B --> C[Memory and recursion]
    C --> D[Prediction and action]
    D --> E[Feedback and recompression]
    E --> A
```
...
```mermaid
flowchart TD
    A["Energetic ceiling: R ≤ E / JCT"] --> C[Combined constraint]
    B["Governance ceiling: R × C ≤ S"] --> C
    C --> D[Safe Recursion Envelope]
    D --> E[Bounded recursive operation]
```
...
```mermaid
flowchart LR
    A[Compression improvement] --> B[Reduced declared burden]
    B --> C[Constraint threshold crossed]
    C --> D[Expanded recursion capacity]
```
...
```mermaid
flowchart TD
    A[Compressed information] --> B{Required structure preserved?}
    B -->|Yes| C[Durable reusable infrastructure]
    B -->|No| D[Lossy or unstable representation]
```
...
```mermaid
flowchart TD
    A[Compressed representation] --> B[Declared prediction]
    B --> C[Bounded evaluation]
    C --> D[Evidence and failure review]
    D --> E[Retain, revise, or reject]
```
...
```mermaid
flowchart TD
    A[Canonical status] --> B[Implementation status]
    B --> C[Benchmark status]
    C --> D[Empirical-validation status]
```
...
```mermaid
flowchart TD
    A[Proposed cross-domain comparison] --> B[Declare objects and scale]
    B --> C[Declare normalization and constraints]
    C --> D[Evaluate evidence and alternatives]
    D --> E[Accept, bound, revise, or reject]
```
...
```mermaid
flowchart LR
    A[Utility and fidelity] --> C[Provisional evaluation]
    B[Cost and risk] --> C
    C --> D[Benchmark refinement]
```
