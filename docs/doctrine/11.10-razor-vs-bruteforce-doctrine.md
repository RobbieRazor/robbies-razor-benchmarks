...
mrd_section: "MRD v1.9 §11.10 (Applied Governance Layer)"
...
**Derived from:** MRD v1.9 §11 (Meta-Recursion Architecture): §11.2, §11.4, §11.4.1, §11.6A, §11.6B, §11.9, §11.11.
...
**Applied diagnostic:** See `docs/diagnostics/economic-recursion-barrier-diagnostic.md` (derived from MRD §11.11) for a quick yes/no test to detect economic recursion failure at scale.
...
4) **Enforce truth as structural invariance** (MRD §11.4.1): prefer representations that remain stable under recursive re-compression; treat thrashing/contradiction as an architectural signal requiring memory binding, not output smoothing.  
5) **Demonstrate economic recursion closure** (MRD §11.11): show that recursion cost per stabilized outcome declines over time and does not require ongoing external subsidy or deferred accounting to remain viable.
6) **Gate recursion depth** with explicit stabilizers (doubt/repair/meaning checks)
