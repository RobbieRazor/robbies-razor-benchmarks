...
class SelectiveReplayBuffer:
    ...
    Scoring intuition (Razor-aligned):
    ...
    Notes:
    - This is a reference buffer. Real systems may store embeddings,
      metadata, provenance, or task IDs.
    - Sampling is weighted by softmax(score), implemented with stdlib.
    """
    ...
    def add_example(
        ...
        # Enforce capacity by keeping highest-score items
    ...
    def _softmax_probs(self) -> List[float]:
        """
        Convert scores into sampling probabilities via softmax.
        Uses numerical stabilization by subtracting max score.
        """
        if not self._buffer:
            return []
